...
In a typical \gls{PDH} setup, a frequency generator is fed to the \gls{EOM} and to a mixer connected to the output of an \gls{RF} photodetector placed in reflection of the cavity (see Figure\,\ref{fig:prototype-setup}). This ensures that the same frequency used to create the \gls{RF} sidebands is used to demodulate the superposition of fields reflecting from the cavity. Mixing the oscillator's signal with the reflected light is equivalent to multiplying the reflected field by a factor of $\sin{\omega t}$ or $\cos{\omega t}$, which yields amplitude modulation proportional to the cavity motion at the frequency of the mirror motion. The key feature of this amplitude modulation is that it is bipolar about the Airy peak, meaning that mirror motion in a certain direction yields amplitude modulation with a certain sign. This \emph{error signal} can be fed back to the cavity's actuators to hold the cavity at the top of the Airy peak. Note here that the \gls{PDH} technique allows the cavity to be held at the top of the Airy peak and not to the side as with the DC locking technique described earlier.
...
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{graphics/generated/from-python/30-temperature-servo-tf.pdf}
  \caption{\label{fig:temperature-servo-tf}Temperature servo transfer function.}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{graphics/generated/from-python/30-pzt-servo-tf.pdf}
  \caption{\label{fig:pzt-servo-tf}Estimated open loop transfer function of the analogue servo used to feed the cavity error signal back to the PZT. The feedback is flat below \SI{100}{\hertz}, where the temperature feedback servo performs the majority of the actuation. Above this point, a slope proportional to $\frac{1}{f^3}$ removes feedback at higher frequencies to limit the bandwidth, and hence provide higher low frequency gain. The slope of the servo is approximately $\frac{1}{f}$ above xxx to provide a stable unity gain crossing.}
\end{figure}
