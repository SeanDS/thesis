...
The key features of \gls{PDH} are highlighted below. More detailed descriptions can be found in, for example,~\cite{Freise2010} and~\cite{Black2001}. Mirror motion imparts phase modulation upon the carrier as shown in \cref{sec:signal-sidebands}. Restating \cref{eq:field-phase-mod-expanded} we see that phase modulation upon the carrier produces upper and lower sidebands with frequencies $\omega_0 \pm \omega$, where $\omega$ represents the phase modulation frequency:
\begin{equation}
...
\end{equation}
The numerical simulation tool \emph{\gls{FINESSE}} (see \cref{sec:finesse-sim}) was then used to calculate $\kappa$ for the cavity parameters shown in Table~\ref{tab:cavity-parameters}. This was determined to be \SI{18.5}{\meter \per \radian}. The \gls{WGM}'s transverse displacement was then the product of $\kappa$ and $\theta$.
