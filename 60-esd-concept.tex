\chapter{\label{c:esd-concept}Techniques for the control of a plate capacitor electrostatic actuator with reduced seismic coupling}
...
This parallel plate capacitor \gls{ESD} has potential applications as an actuator for test masses in suspended interferometers. It was shown by Wittel \etal{} \cite{Wittel2015} that the force provided by plate capacitors with dimensions applicable to the \AEIPROTOTYPE{} is around \SI{1.5}{\micro\newton} at \SI{1}{\kilo\volt} corresponding to a displacement of around \SI{0.3}{\micro\meter}. This confirms that the \gls{ESD} is only suitable for small corrections, and because the suspension systems used in suspended interferometers filter ground motion to a greater extent at high frequency, this type of actuation tends to lend itself more to the control of radiation pressure and other displacement effects at frequencies above \SI{50}{\hertz} where seismic motion is typically insignificant.

\subsection{Test of an electrostatic actuator to inform the \SSMEXPT{}}
The plan for the \SSMEXPT{} is to adopt a plate capacitor design for the actuation of the \glspl{ETM} to take advantage of its benefits. A number of potential issues have been highlighted with this \gls{ESD} design that require a dedicated experiment to test before these actuators can be used in the main \SSMEXPT{}.

The effect of misaligned plates, cross-couplings to yaw etc.
Effect of plate separation
Positioning of the test mass between the plates
Control and noise of high voltage signals
Signal routing in vacuum

The geometry of the intended experimental apparatus are shown in Table\,\ref{tab:esd-expt-parameters}.
