...
The effective displacement noise the \gls{HV} amplifier would produce on the \gls{ETM} suspensions can be calculated following the same process used to create the maximum \gls{ETM} displacement shown in Figure\,\ref{fig:ssm-etm-disp-esd-max}. Assuming that the force-voltage gradient is \SI{-3.68}{\nano\newton\per\volt} at \SI{750}{\volt} as shown in Figure\,\ref{fig:ssm-esd-ansys}, we can calculate the gradient for voltages approximating the noise. The noise voltage at \SI{100}{\hertz} is around \SI{30}{\micro\volt}. The displacement noise at \SI{100}{\hertz} where the measurement of quantum radiation pressure noise may be made is then given by the product of the \gls{ESD}'s force per volt, the \gls{ETM} suspension's displacement per unit force and the noise voltage.

CHECK THIS We can use the force per volt given by the fit shown in Figure\,\ref{fig:ssm-esd-ansys} modified by the square of the voltage difference, i.e. $\SI{3.68}{\nano\newton\per\volt} \left( \frac{\SI{30}{\micro\volt}}{\SI{750}{\volt}} \right)^2 = \SI{6e-24}{\newton\per\volt}$. The displacement at \SI{100}{\hertz} is around \SI{10}{\micro\meter\per\newton} and so the effective displacement is almost \SI{e-30}{\meter\per\sqrthz}, which is far below the noise budget for the \SSMEXPT{} shown in Figure\,\ref{fig:noise-budget}.
