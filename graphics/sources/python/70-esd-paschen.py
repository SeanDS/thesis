...
for i in range(len(d)):
    ax1.loglog(p * 1e2, V[i], color=colours.next(), alpha=lf.ALPHA_LINE_A)

# horizontal line representing maximum voltage
ax1.hlines(750, (p*1e2).min(), (p*1e2).max(), colors=colours.next(), linestyles='dashed', zorder=2)

# vertical line representing atmospheric pressure
ax1.vlines(1e8, 1e1, 1e6, colors=colours.next(), linestyles='dashed', zorder=2)
