...
prcl_angle = 55 # eyeball, opposite sign due to coupling
...
# override legend padding
#with plt.rc_context({'legend.borderaxespad': 0.5}):
ax3.legend([r'MICH to $\mathrm{REFL}_{\mathrm{57}}$', r'PRCL to $\mathrm{REFL}_{\mathrm{57}}$', 'Operating point'], loc='upper left', framealpha=0.8)
...
mich_angle = 50 # eyeball, opposite sign due to coupling
...
# override legend padding
#with plt.rc_context({'legend.borderaxespad': 0.5}):
ax4.legend([r'PRCL to $\mathrm{POP}_{\mathrm{11}}$', r'MICH to $\mathrm{POP}_{\mathrm{11}}$', 'Operating point'], loc='upper left', framealpha=0.8)
...
# combine I and Q
srcl_angle = -30 # eyeball
mich_angle = 50 # eyeball, opposite sign due to coupling
data_srcl_combined = data_srcl[:, 1] * np.sin(np.radians(srcl_angle)) + data_srcl[:, 2] * np.cos(np.radians(srcl_angle))

# plot
ax4.plot(data_srcl[:, 0] * x_scale_factor_srcl, data_srcl_combined * y_scale_factor_srcl, '-', color=colour_a, alpha=lf.ALPHA_LINE_A)

# vertical line for operating point
ax4.vlines(0, -1 * y_scale_factor_srcl, 1 * y_scale_factor_srcl, colors=colour_line, linestyles='dashed', zorder=2)

ax4.set_ylabel('Power (mW)')
ax4.set_xlabel('Offset (nm)')

ax4.set_xlim([-1e-8 * x_scale_factor_srcl, 1e-8 * x_scale_factor_srcl])
ax4.set_ylim([-0.5e-3 * y_scale_factor_srcl, 0.5e-3 * y_scale_factor_srcl])

ax4.grid(True)

# override legend padding
#with plt.rc_context({'legend.borderaxespad': 0.5}):
ax4.legend([r'SRCL to $\mathrm{REFL}_{\mathrm{45}}$', r'MICH to $\mathrm{POP}_{\mathrm{11}}$', 'Operating point'], loc='upper left', framealpha=0.8)
