...
data_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data', '30-posterior-stddev.csv')
...
ax.plot(stddev[:, 0], stddev[:, 1], color=colours.next(), lw=2)

ax.grid(True)

ax.xaxis.set_major_formatter(FixedOrderFormatter(-4))

ax.set_xlabel('Spot Movement Standard Deviation [m]')
