...
\note{Should add something about gamma here, the half-bandwidth of a cavity, for chapter 4}
...
\section{Simulating interferometer response}
Light propagating within an interferometer will collect signal terms in reflection from and transmission through each encountered mirror, and the way in which the signal depends on the motion of the mirrors is termed the \emph{response}. While in certain circumstances the analytical calculation of a particular response is straightforward, such as with the basic \FP{} interferometer described in Section\,\ref{sec:simple-cavities}, the algebra representing more complex interferometers quickly becomes extremely cumbersome. For anything beyond the simplest of examples, it is usually beneficial to employ numerical simulation tools to perform the tedious calculations involved in obtaining the output signals from interferometers as optics are perturbed, with the packages Finesse \cite{Freise2004} and Optickle \cite{Evans2012} being popular choices. These tools are discussed in greater detail in Appendix\,\ref{a:simulation-tools}.