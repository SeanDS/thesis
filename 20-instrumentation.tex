...
The field due to motion of a mirror $\Delta x$ at a distance $x$ can be determined by combining Equations \ref{eq:field-amplitude} and \ref{eq:reflected-field}. As the light must travel distance $2x$ to get to and from the optic, the field picks up a factor of $\text{e}^{2\text{i}k \left( x + \Delta x \right)}$ in phase and a factor of $\text{i}r$ in amplitude:
\begin{equation}
  E_{\text{r}} \left( x \right) = \text{i} r E_0 \text{e}^{2\text{i} k \left( x + \Delta x \right)}.
\end{equation}
...
\begin{equation}
  ...
  E_{\text{r}} \left( t \right) = \text{i} r E_0 \text{e}^{\text{i} \left( 2 \omega_0 t + k \Delta x\right)}.
\end{equation}
...
\end{equation}
where we introduce $m$ as the \emph{modulation depth}, a dimensionless number expressing the strength of the mirror motion. Note that we have neglected for now the reflection terms $ir$. With some algebraic manipulation involving Bessel functions, we can express this as \cite{Freise2010}:
\begin{equation}
...
Amplitude modulation has a similar but not identical effect to phase modulation. Devices such as electro-optic modulators (\glspl{EOM}) can perform amplitude modulation on a light field, and the effect can be expressed again in terms of the modulation depth and frequency:
\begin{equation}
...
Recall the reflection terms $ir$ we neglected earlier. Upon reflection, Equation\,\ref{eq:field-phase-mod-expanded} gains an imaginary coefficient and so the phase modulation becomes amplitude modulation. When the wave next reflects from a mirror, this amplitude modulation again becomes phase modulation. Similarly, amplitude modulation shown in Equation\,\ref{eq:field-amp-mod} is converted to phase modulation upon reflection. When these signals are measured by a photodetector, the number of mirrors a wave has reflected from, however, does not matter: to the photodetector, amplitude and phase modulation are indistinguishable. There is an interesting caveat to highlight here: if amplitude modulation of a field produces only two sidebands, whereas phase modulation produces infinitely many, how can they express the same signal on a photodetector? To resolve this apparent inconsistency it is useful to visualise the signal sidebands in a phasor diagram, as shown in Figure XXX.
