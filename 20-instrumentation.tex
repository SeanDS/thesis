...
\subsection{Planned detector upgrades and new facilities}
...
The effect squeezing has on the quantum noise spectral density of a \MI{} can be described in terms of a modification to Equation\,\ref{eq:classicalifospectrum} by a factor xxx...:
\note{equation}
...
\subsubsection{Variational readout}
...
\subsubsection{Light-mirror interactions}
...
One example is through \emph{optical bars} which couple the motion of heavy mirrors, providing the \gls{SQL} limit, to a light mirror for which a local measurement can be performed \cite{Braginsky1997}. This effectively reduces the back-action applied to the mirror by the measurement photons, but can in principle preserve the sub-\gls{SQL} behaviour of the heavy mirror.

The complicated control requirements for this type of interferometer are such that these techniques are not currently considered for future detectors.
