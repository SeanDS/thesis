...
The dominant contribution to coating noise in current generation gravitational wave detectors, Brownian noise, has a power spectral density given as \cite{Harry2002}
\begin{equation}
...
\end{equation}
in units of \SI{}{\meter^2\per\hertz}, for power $P$ and laser angular frequency $\omega_0$. As this noise arises from  spontaneous creation and annihilation of photons in space, it is a statistical random process and so the spectral density has equal power at all frequencies; it is \emph{white}. The strain-equivalent power spectral density is Equation\,\ref{eq:shot-noise-psd} normalised to the arm length $L$:
\begin{equation}
...
\end{equation}
with $P_0$ the laser power at the test masses, $P_{SQL}$ the laser power required to reach the \gls{SQL} at the cavity pole frequency and $\gamma$ the arm cavity half-bandwidth. $P_{SQL}$ is given by \cite{Kimble2001}
\begin{equation}
