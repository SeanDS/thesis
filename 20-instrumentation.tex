...
The field due to motion of a mirror at a distance $x$ can be determined by combining Equations \ref{eq:field-amplitude} and \ref{eq:reflected-field}. As the light must travel to and from the optic, the field picks up a factor of $\text{e}^{2\text{i}kx}$ in phase, and a factor of $\text{i}r$ in amplitude. Distance $x$ may in general be a time varying function, so the time varying field is then:
\begin{equation}
  E \left( t \right) = \text{i} r E_0 \text{e}^{\text{i} k x \left( t \right)}.
\end{equation}
This lets us calculate the effect of mirror motion on the field. If we assume the motion of the mirror is sinusoidal (usually a reasonable approximation, given that mirrors are, in the field of gravitational wave detection, typically suspended from pendulums), then the field can be expressed in terms of the peak amplitude $x_0$ and frequency $f$ of oscillation:
\begin{equation}
  E
\end{equation}
\note{See p23 of Living Review, but explain phase modulation in terms of x(t).}
