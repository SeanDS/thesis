...
A major concern for designers of metrological experiments is that of quantum noise. These are unwanted signals which appear in instruments uncorrelated with the signals intended to be measured. As such, the presence of quantum noise in an instrument degrades the precision to which the intended measurement can be made.
...
\end{equation}
and this means that the observables contain correlated components and as such cannot be considered entirely separate entities. Measuring one form automatically influences the other, leading to the well-known \emph{Heisenberg Uncertainty Principle}:
\begin{equation}
...
\end{equation}
The key result to highlight here is that, even with an otherwise unperturbed mirror, the momentum imparted by the measurement at time $t$ influences the later measurement at $t + \tau$ in such a way that it adds uncertainty. Or, put another way, Equation\,\ref{eq:heisenburguncertainty} expresses that the smaller the error on the position of an observable in a quantum mechanical system, the greater the error on the momentum; and vice-versa. The same result holds for $n$ and $\phi$.

Quantum shot noise and quantum radiation pressure noise limit the sensitivity of classical interferometers. The former effect arises from counting statistics: lasers, despite producing a constant average number of photons, do not produce equally-spaced photons; the finite sampling rate of photodetectors leads to a fluctuation in the output signal corresponding to the fluctuation of the laser's output. Each sample the photodetector takes will yield a fluctuating power reading even when the average incident power is constant. Radiation pressure noise arises from the effect of momentum transfer between photons and reflective surfaces within interferometers, as shown by Equation\,\ref{eq:heisenburgtime}. Upon reflection, a photon imparts momentum \note{how much?} to the mirror, and the next photon to hit the mirror undergoes a different optical path length to the initial photon.

Shot noise is present with equal power at all frequencies as it arises from spontaneous pair production in vacuum.

The upward trend in shot noise seen in the sensitivity curve in Figure xxx arises from the cavity dynamics of the system. The white shot noise is sampled less as the cavity storage time....
...
\note{Quantum noise is low enough that its self radiation pressure effect is negligible. However, the DC radiation pressure effect caused by the classical light causes the quantum noise to be shaped by the cavity, meaning that the quantum noise on a probe is not necessarily flat. This might be described in Caves? Or KLMTV?}
